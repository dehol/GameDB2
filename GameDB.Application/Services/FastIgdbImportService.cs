...
public class FastIgdbImportService
{
    ...
    private async Task ProcessSingleGameAsync(int appId, IGameRepository games, CancellationToken ct)
    {
        ...
        try
        {
            var query = $"search \"{game.Name.Replace("\"", "")}\"; fields name, summary, storyline, first_release_date, rating, rating_count, cover.image_id, genres.name, involved_companies.company.name, involved_companies.developer, involved_companies.publisher; limit 1;";
            ...
            {
                _logger.LogWarning("⚠️ [IGDB] Гру не знайдено: {Name}", game.Name);
            ...
            if (igdbData.genres != null)
            {
                ...
                {
                    var genre = await games.GetOrCreateGenreAsync(g.name!, g.name!);
