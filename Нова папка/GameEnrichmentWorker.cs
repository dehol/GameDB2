            ...
            try
            {
                ...
                while (_state.IsImporting && !stoppingToken.IsCancellationRequested)
                {
                    var appIds = _state.OverwriteExisting
                        ? await gamesRepo.GetSteamAppIdsBatchAsync(_state.OverwriteSkip, batchSize, stoppingToken)
                        : await gamesRepo.GetAppIdsWithoutDetailsAsync(batchSize, stoppingToken);
                    if (appIds.Count == 0)
                    {
