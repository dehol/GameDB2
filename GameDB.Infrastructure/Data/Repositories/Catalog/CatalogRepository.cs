...
/// <summary>
/// Оптимізований репозиторій каталогу.
///
/// ╔══════════════════════════════════════════════════════════════╗
/// ║  Що змінилось і чому — стисло                               ║
/// ╠══════════════════════════════════════════════════════════════╣
/// ║  GetPagedAsync                                              ║
/// ║  БУЛО: Include×4 + AsSplitQuery → 5 round-trips            ║
/// ║        + entity-graph materialize + MapToCard у C#          ║
/// ║  СТАЛО: 2 round-trips (COUNT + projection)                  ║
/// ║         BestOffer → LATERAL JOIN (EF Core 8 / Npgsql)      ║
/// ║                                                              ║
/// ║  GetDetailAsync                                             ║
/// ║  БУЛО: Include×5 + AsSplitQuery → 5 round-trips            ║
/// ║  СТАЛО: 1 projection query                                  ║
/// ║                                                              ║
/// ║  GetPriceHistoryAsync                                       ║
/// ║  БУЛО: Include(External+Shop) + Include(PriceHistories)     ║
/// ║  СТАЛО: пряма проекція; WHERE Any() до Select               ║
/// ╚══════════════════════════════════════════════════════════════╝
/// </summary>
public sealed class CatalogRepository(AppDbContext db) : ICatalogRepository
{
    // ═════════════════════════════════════════════════════════════════════════
    //  GetPagedAsync — головний гарячий шлях
    // ═════════════════════════════════════════════════════════════════════════

    /// <summary>
    /// Два SQL-запити:
    ///   1. COUNT(*) на відфільтрованому наборі (без ORDER BY → дешевий план).
    ///   2. Projection query: ORDER BY → Skip/Take → SELECT із LATERAL JOIN
    ///      для best offer. Нуль Include, нуль entity graphs, нуль AsSplitQuery.
    ///
    /// BestOffer subquery:
    ///   EF Core 8 + Npgsql транслює SelectMany + OrderBy + FirstOrDefault
    ///   у LEFT JOIN LATERAL (...) ON TRUE з ORDER BY + LIMIT 1.
    ///   PostgreSQL виконує це через INDEX ONLY SCAN на IX_GameOffer_ExternalId_Cover
    ///   (≤3 рядки, тільки 3 магазини) — фактично константний час.
    ///
    /// Genres subquery:
    ///   Correlated EXISTS + JOIN на GameGenre/Genre — виконується лише
    ///   для PageSize (24) рядків, прийнятно.
    /// </summary>
        ...
        CatalogFilterDto f, CancellationToken ct = default)
    {
        ...
        // ── Query 1: COUNT ───────────────────────────────────────────────────
        // На базовому запиті без ORDER BY — PostgreSQL вибирає найдешевший план
        // (часто Index Only Scan на IX_Game_ImportStatus або partial index).
        ...
        // ── Query 2: Projection ──────────────────────────────────────────────
        // Один SQL з LATERAL JOIN для best offer.
        // AsNoTracking() вже є на baseQuery — не потрібен повторно.
        var rows = await ApplySorting(baseQuery, f)
            ...
            .Select(g => new
            {
                ...
                // LEFT JOIN Developer (nullable FK) — EF Core генерує автоматично
                DeveloperName = (string?)g.Developer!.Name,

                // JOIN GameGenre + Genre: correlated subquery для 24 рядків
                Genres = g.Genres
                    .OrderBy(gr => gr.Name)
                    .Select(gr => gr.Name)
                    .ToList(),

                // ── LATERAL JOIN для best offer ──────────────────────────────
                // EF Core 8 транслює SelectMany(outerKey, resultSelector)
                //   + OrderBy + FirstOrDefault
                // у LEFT JOIN LATERAL (SELECT ... ORDER BY ... LIMIT 1) ON TRUE.
                //
                // Tie-break по GameOfferId (PK) — детермінований вибір
                // при однакових FinalPrice у різних магазинах.
        ...
        var items = rows.Select(r => new CatalogGameDto(
            ...
            Genres:           r.Genres,
            BestFinalPrice:   r.BestOffer?.FinalPrice,
            BestCurrentPrice: r.BestOffer?.CurrentPrice,
            BestDiscount:     r.BestOffer?.CurrentDiscount ?? 0,
            BestCurrency:     r.BestOffer?.Currency,
            BestShopName:     r.BestOffer?.ShopName,
            BestDownloadUrl:  r.BestOffer?.ExternalUrl,
    ...
    // ═════════════════════════════════════════════════════════════════════════
    //  ApplySorting — виключно server-side ORDER BY, без client-side оцінки
    // ═════════════════════════════════════════════════════════════════════════

    /// <remarks>
    /// Scalar sorts (Name, ReleaseDate, Rating, UpdatedAt, Popularity):
    ///   Прямі стовпці Game — ORDER BY без підзапитів.
    ///   Popularity = Rating * RatingCount: scalar expression у SQL,
    ///   частковий функціональний індекс IX_Game_Popularity покриває його.
    ///
    /// Price / Discount:
    ///   Correlated MIN/MAX subquery на GameExternalId → GameOffer.
    ///   З покриваючим індексом IX_GameOffer_ExternalId_Cover PostgreSQL виконує
    ///   INDEX ONLY SCAN на ≤3 рядках (лише 3 магазини) → ефективно O(1) per row.
    ///   Якщо каталог зростає до 50k+ ігор і Price-sort все ще повільний —
    ///   переходьте на materialized view mv_game_best_offer (DDL у catalog_indexes.sql)
    ///   і замініть ці два case на JOIN по view.
    ///
    /// ThenBy(GameId): забезпечує стабільну пагінацію при однакових значеннях
    ///   ключа сортування (PostgreSQL не гарантує порядок без явного tie-break).
    /// </remarks>
